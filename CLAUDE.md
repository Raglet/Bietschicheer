...
- `404.html` — the QR-code scan landing page (custom-404-page fallback trick, see "Running & deployment"). See "Bietschimeile stamp card" below — not linked from anywhere in the UI, only reached via a printed QR code.
...
- **`counter/index.html`** + **`counter/counter.js`** — a hidden, unlinked page for entry staff to count guests: big live total, `+1` / `+3` / `+5` / `+10` buttons and a `−1`/`−3`/`−5`/`−10` correction row. Staff reach it only via the short URL `…/counter` (a folder with `index.html`; the bare path is redirected to `/counter/` by `vercel.json`, see "Running & deployment" — needed because the page's own asset paths are `../`-relative, which only resolve correctly once the browser sees a trailing slash; copyable in the admin "Gäste" tab).
...
- **Admin tool** (`admin/index.html`, reachable at the short URL `…/admin` — folder-index trick, same as `counter/`, with the same `vercel.json` redirect to the trailing slash; renamed from `admin/admin.html`) edits Firestore directly — writes are live immediately, no export/copy/paste/commit step (that workflow from an earlier iteration is gone). Login is **Firebase Auth Google Sign-In**, not a password: after sign-in, the tool checks the `admins` collection for a doc matching the signed-in email (`AdminCore.checkAdminAccess`); if absent, it signs the account back out. There is no app-managed password anywhere in this tool anymore.
...
- Deployed as static files. The live custom domain (`live.bietschicheer.ch`) is served by **Vercel** (a linked project, `.vercel/project.json` — gitignored, local-only), auto-deploying on push; a project may also exist on GitHub Pages for a `github.io` URL, but that's not the one referenced by name anywhere in this doc. Geolocation ("Du bist hier") and QR scanning need HTTPS, which both hosts provide; they won't prompt on `file://`. The deployed domain must also be added to Firebase's Authorized domains, or Google Sign-In will fail there.
- `vercel.json` (repo root) redirects the bare `/admin` and `/counter` paths to their trailing-slash form (`/admin/`, `/counter/`) — needed because those pages load their assets via `../`-relative paths (kept relative so the same page works from a GitHub Pages project subpath too), which only resolve correctly once the browser's URL for the page actually ends in `/`. Update this file if another folder-index page (`…/foo/index.html`) is added and needs a short `…/foo` URL.
